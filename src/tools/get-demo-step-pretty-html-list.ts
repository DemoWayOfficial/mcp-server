...
- "url": the step HTML file URL, you can use any other tool you have to fetch the HTML's content. It's important to note that HTML content is always particularly large, so it's best to handle it by downloading the file locally or reading it in segments.`;

const Schema = z.object({
  demoId: z.string().describe('DemoWay demo ID'),
});

const executer = createToolExecuter<typeof Schema.shape>(async ({ demoId }, extra) => {
...
export function registerGetDemoStepPrettyHTMLListTool(server: McpServer) {
